...
def service(mock_vector_store, mocker):
    ...
    mocker.patch("app.core.services.retrieval_service.DuckDuckGoSearchRun")
    retrieval_svc = RetrievalService(vector_store=mock_vector_store, similarity_threshold=0.4)
...
async def test_get_context_happy_path_internal_data(service, mock_vector_store):
    ...
    doc1 = Document(page_content="NEPSE index gained 10 points today.", metadata={"source": "finance_report.pdf"})
    doc2 = Document(page_content="Commercial banking sectors show low volatility.", metadata={"source": "finance_report.pdf"}) # Duplicate source
    doc3 = Document(page_content="Random text chunk.", metadata={"file_name": "backup_data.txt"}) # Alternate metadata key
...
@pytest.mark.asyncio
async def test_get_context_edge_case_below_threshold_triggers_fallback(service, mock_vector_store):
    """
    ...
    weak_doc = Document(page_content="Outdated irrelevant information.", metadata={"source": "old_cv.txt"})
    mock_vector_store.asimilarity_search_with_relevance_scores.return_value = [(weak_doc, 0.38)]
    
    # Mock DuckDuckGo returning data
    service._web_search_tool.arun.return_value = "Live web search summary snippet about Kathmandu."

    context_str, is_internal, sources = await service.get_context("Current Kathmandu weather")
...
async def test_get_context_edge_case_missing_metadata_keys(service, mock_vector_store):
    ...
    mock_vector_store.asimilarity_search_with_relevance_scores.return_value = [(naked_doc, 0.90)]
...
@pytest.mark.asyncio
async def test_get_context_failure_cascading_infrastructure_breakdown(service, mock_vector_store):
    """
    ...
    mock_vector_store.asimilarity_search_with_relevance_scores.side_effect = Exception("Chroma connection refused")
    
    # 2. Force DuckDuckGo Search Engine to crash right after
    service._web_search_tool.arun.side_effect = Exception("DuckDuckGo rate limit exceeded (429)")

    # Execution should not raise an error due to your try/except block layouts
    context_str, is_internal, sources = await service.get_context("Emergency test question")
